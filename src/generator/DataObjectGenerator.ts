...
export class DataObjectGenerator {
    ...
    static createJavaClass(object: ObjectMetadata, version = 1): [string, string] {
        ...
        const hasPrimaryKey =
            // `options` is only populated for members that were given decorator options,
            // so it is absent on plain @SerializableMember declarations. It happened to
            // be present on every member of @openhps/core 0.3.49; on 1.0.16 it is not,
            // and the unguarded cast crashed the generator during postinstall.
            ...
            `package ${packageStr};\n\n` +
                ...
                dataMembers
                    .map((member) => {
                        ...
                        const options: SerializableMemberOptions | undefined = member.options;
                        return (
